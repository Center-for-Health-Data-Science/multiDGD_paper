...
def predict_perturbations_internal(model, rep, correction, dataset, modality_id, feature_id, n_epochs=1, direction="down", fold_change=10):
    """predict the effect of upregulating or silencing a feature fold_change times"""
    # n_samples = dataset.data.shape[0]
    # define rep optimizer
    rep_optimizer = torch.optim.Adam(rep.parameters(), lr=1e-2, weight_decay=1e-4, betas=(0.5, 0.7))
    ...
    if modality_id == 1:
        if direction == "down":
            ...
            x_perturbed[:, feature_id+model.train_set.modality_switch] = (x_perturbed[:, feature_id+model.train_set.modality_switch] + 1) * fold_change
    else:
        if direction == "down":
            x_perturbed[:, feature_id] = 0
        else:
...
def predict_perturbations(model, testset, feature_id, split="test", new_data=False, direction="down", fold_change=10):

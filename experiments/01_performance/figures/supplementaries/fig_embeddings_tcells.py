...
for i, data_name in enumerate(data_names):
    ...
    if not os.path.exists('results/analysis/performance_evaluation/dgd_'+data_name+'_TCELL_umap.csv'):
        ...
        t_cell_string = '+ T'
        t_cell_indices = [i for i, s in enumerate(trainset.obs['cell_type'].values) if t_cell_string in s]
        ...
        plot_data.to_csv('results/analysis/performance_evaluation/dgd_'+data_name+'_TCELL_umap.csv', index=False)
    else:
        plot_data = pd.read_csv('results/analysis/performance_evaluation/dgd_'+data_name+'_TCELL_umap.csv')
        plot_data['cell type'] = plot_data['cell type'].astype("category")
        plot_data['cell type'] = plot_data['cell type'].cat.set_categories(cluster_class_neworder)
...
for i, data_name in enumerate(data_names):
    ...
    cluster_class_neworder, class_palette = make_palette_from_meta(data_name)
    column_names = ['UMAP D1', 'UMAP D2']
    if not os.path.exists('results/analysis/performance_evaluation/mvi_'+data_name+'_TCELL_umap.csv'):
        print('making umap for data '+data_name)
        is_train_df = pd.read_csv('data/'+data_name+'/train_val_test_split.csv')
        if data_name != 'human_brain':
            trainset, testset, modality_switch, library = load_testdata_as_anndata(data_name)
        ...
        plot_data.to_csv('results/analysis/performance_evaluation/mvi_'+data_name+'_TCELL_umap.csv', index=False)
    else:
        plot_data = pd.read_csv('results/analysis/performance_evaluation/mvi_'+data_name+'_TCELL_umap.csv')
        plot_data['cell type'] = plot_data['cell type'].astype("category")
        plot_data['cell type'] = plot_data['cell type'].cat.set_categories(cluster_class_neworder)
...
figure_name = "results/analysis/plots/fig_embeddings_supp_marrow_tcells"

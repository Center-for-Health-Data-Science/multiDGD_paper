...
ax_list[-1].text(grid_letter_positions[0]-0.4, 1.+grid_letter_positions[1],
...
def mvi_confusion_matrix(mod, rep, labels, true_labels, cluster_labels, class_order=None, norm=True):
    ...
    if class_order is None:
        ...
        class_order_num = [np.where(classes == class_order[i])[0][0] for i in range(len(class_order))]
...
if not os.path.exists('results/analysis/performance_evaluation/mvi_'+data_name+'_cluster_map.csv'):
    ...
    df_relative_clustering = mvi_confusion_matrix(model, trainset.obsm['latent'], trainset.obs[plotting_key[i]].values, true_labels, cluster_labels, class_order)
    df_relative_clustering.to_csv('results/analysis/performance_evaluation/mvi_'+data_name+'_cluster_map.csv')

    df_clustering = mvi_confusion_matrix(model, trainset.obsm['latent'], trainset.obs[plotting_key[i]].values, true_labels, cluster_labels, class_order, norm=False)
...
plt.savefig('results/analysis/plots/performance_evaluation/fig_supp_heatmaps_marrow_mvi_dgd.png', dpi=300, bbox_inches='tight')
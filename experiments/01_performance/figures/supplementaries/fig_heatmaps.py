...
ax_list[-1].text(grid_letter_positions[0]-0.3, 1.+grid_letter_positions[1],
...
if not os.path.exists('results/analysis/performance_evaluation/'+data_name+'_cluster_map.csv'):
    ...
    df_relative_clustering.to_csv('results/analysis/performance_evaluation/'+data_name+'_cluster_map.csv')
else:
    df_relative_clustering = pd.read_csv('results/analysis/performance_evaluation/'+data_name+'_cluster_map.csv', index_col=0)
if not os.path.exists('results/analysis/performance_evaluation/'+data_name+'_cluster_map_nonorm.csv'):
    is_train_df = pd.read_csv('data/'+data_name+'/train_val_test_split.csv')
    trainset, testset, modality_switch, library = load_testdata_as_anndata(data_name)
    model = DGD.load(data=trainset,
            save_dir=save_dir+data_name+'/',
    ...
    df_clustering.to_csv('results/analysis/performance_evaluation/'+data_name+'_cluster_map_nonorm.csv')
else:
    df_clustering = pd.read_csv('results/analysis/performance_evaluation/'+data_name+'_cluster_map_nonorm.csv', index_col=0)
...
ax_list[-1].set_title('percentage of cell type represented by GMM cluster ('+data_name+')')
#"""
...
# label the first row as C
ax_list[-1].text(grid_letter_positions[0]-0.5, 1.+grid_letter_positions[1],
...
ax_list[-1].set(yticklabels=ylabels)
# rotate y labels
plt.setp(ax_list[-1].get_yticklabels(), rotation=0, ha="right",
...
plt.savefig('results/analysis/plots/performance_evaluation/fig_supp_heatmaps.png', dpi=300, bbox_inches='tight')
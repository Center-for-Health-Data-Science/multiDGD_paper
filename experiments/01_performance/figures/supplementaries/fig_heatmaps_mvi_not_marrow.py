...
plotting_key = ["celltype", "celltype"]
...
for i, data_name in enumerate(data_names):
    ...
    if not os.path.exists(metric_dir + "mvi_" + data_name + "_cluster_map.csv"):
        ...
        df_relative_clustering.to_csv(
            metric_dir + "mvi_" + data_name + "_cluster_map.csv"
        )
        ...
        df_clustering.to_csv(
            metric_dir + "mvi_" + data_name + "_cluster_map_nonorm.csv"
        )
    else:
    ...
    if i > 0:
        extra_dist = 0.5
    ax_list[-1].text(
        grid_letter_positions[0] - extra_dist,
    ...
    ax_list[-1].set_title(
        "percentage of cell type represented by Leiden cluster (" + data_name + ")"
    )

plt.savefig(
    "../results/analysis/plots/supplementaries/fig_supp_heatmaps_mvi.png",
    dpi=300,
    bbox_inches="tight",
)

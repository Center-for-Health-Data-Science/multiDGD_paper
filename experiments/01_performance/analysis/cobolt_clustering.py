...
for i, seed in enumerate(random_seeds):
    print('seed: ', seed)
    if not os.path.exists('results/analysis/performance_evaluation/cobolt_'+data_name+'_rs'+str(random_seeds[i])+'_umap.csv'):
        if trainset is None:
        ...
        plot_data.to_csv('results/analysis/performance_evaluation/cobolt_'+data_name+'_rs'+str(random_seeds[i])+'_umap.csv', index=False)
    else:
        plot_data = pd.read_csv('results/analysis/performance_evaluation/cobolt_'+data_name+'_rs'+str(random_seeds[i])+'_umap.csv')
    
    ax_list.append(plt.subplot(gs[i]))
    ax_list[-1].set_title('seed '+str(seed))
    ax_list[-1].set_xlabel('UMAP D1')
    ax_list[-1].set_ylabel('UMAP D2')
    sns.scatterplot(data=plot_data, x='UMAP D1', y='UMAP D2', hue='cell type', palette=class_palette, ax=ax_list[-1], s=1, linewidth=0)
    if i == len(random_seeds)-1:
        ax_list[-1].legend(bbox_to_anchor=(1.0, 1.15),
                   loc='upper left',
                   frameon=False,
                   handletextpad=handletextpad*2,
                   markerscale=handlesize,
                   ncol=1,title='brain cell type',
                   labelspacing=0.2)
    else:
...
if len(aris_out) > 0:
    ...
    aris_df.to_csv('results/analysis/performance_evaluation/cobolt_'+data_name+'_aris.csv', index=False)

plt.savefig('results/analysis/plots/performance_evaluation/fig_supp_cobolt_latent.png', dpi=300, bbox_inches='tight')
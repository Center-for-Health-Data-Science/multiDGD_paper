...
def binary_output_scores(target, output, switch, threshold, batch_size=5000, feature_indices=None):
    '''returns FPR, FNR, balanced accuracy, LR+ and LR-'''
    tp, fp, tn, fn = classify_binary_output(target, output, switch, threshold, batch_size, feature_indices)
    tp = tp.sum()
    fp = fp.sum()
    tn = tn.sum()
    fn = fn.sum()
...
def classify_binary_output(target, output, switch, threshold, batch_size=50, feature_indices=None):
...
if not os.path.exists('results/analysis/performance_evaluation/reconstruction/brain_test_counts_gex.npy'):
    # load brain data
    import mudata as md
    ...
    np.save('results/analysis/performance_evaluation/reconstruction/brain_test_counts_gex.npy', test_data['rna'].X.toarray())
    np.save('results/analysis/performance_evaluation/reconstruction/brain_test_counts_atac.npy', test_data['atac'].X.toarray())
else:
    print('data sets already generated')
    test_gex = np.load('results/analysis/performance_evaluation/reconstruction/brain_test_counts_gex.npy')
    test_atac = np.load('results/analysis/performance_evaluation/reconstruction/brain_test_counts_atac.npy')
...
for count, seed in enumerate(random_seeds):
    ...
    recon_gex = np.asarray(mmread('results/analysis/performance_evaluation/reconstruction/scmm_counts_gex_rs'+str(seed)+'.mtx').todense())
    # plot recon vs original
    import matplotlib.pyplot as plt
    plt.scatter(test_gex.flatten(), recon_gex.flatten(), s=1)
    plt.xlabel('original')
    plt.ylabel('reconstructed')
    plt.savefig('results/analysis/plots/performance_evaluation/scmm_counts_gex_rs'+str(seed)+'.png')

    errors = compute_error_per_sample(torch.tensor(test_gex), torch.tensor(recon_gex), reduction_type='ms')
    rmse = torch.sqrt(torch.mean(errors))
    print('RMSE: ', rmse.item())

    errors = compute_error_per_sample(torch.tensor(test_gex), torch.tensor(recon_gex), reduction_type='ma')
    mae = torch.sqrt(torch.mean(errors))
    print('MAE: ', mae.item())

    recon_atac = np.asarray(mmread('results/analysis/performance_evaluation/reconstruction/scmm_counts_atac_rs'+str(seed)+'.mtx').todense())
    threshold = 0.5
    tpr, tnr, balanced_accuracy, positive_likelihood_ratio, negative_likelihood_ratio = binary_output_scores(test_atac, torch.tensor(recon_atac), test_gex.shape[1], threshold)
...
metrics_df.to_csv('results/analysis/performance_evaluation/reconstruction/scMM_brain_recon_performance.csv')

...
def alternative_ATAC_metrics(target, output, axis=0):
    n_samples = target.shape[0]
    x_accessibility = torch.Tensor(target)
    y_accessibility = output
...
for count, seed in enumerate(random_seeds):
    ...
    auprc, spearman = alternative_ATAC_metrics(test_atac, torch.tensor(recon_atac), axis=None)
    print("auprc: ", auprc)
...
metrics_df.to_csv(
    "../results/revision/analysis/performance/scMM_brain_recon_performance_revision.csv"
)

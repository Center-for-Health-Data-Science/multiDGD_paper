...
efficiency_df_mvi = pd.read_csv(
    "results/analysis/performance_evaluation/human_bonemarrow_data_efficiency_mvi.csv"
)
# change multiVI to MultiVI in model column
efficiency_df_mvi["model"] = [
    "MultiVI" if x == "multiVI" else x for x in efficiency_df_mvi["model"].values
]
...
for count, loss in enumerate(efficiency_df_mvi["loss"].values):
    ...
    base_loss = efficiency_df_mvi[
        (efficiency_df_mvi["random_seed"] == rs)
        & (efficiency_df_mvi["fraction"] == 100)
    ]["loss"].item()
    loss_ratios.append(loss / base_loss)
efficiency_df_mvi["loss ratio"] = loss_ratios
efficiency_df = pd.read_csv(
    "results/analysis/performance_evaluation/human_bonemarrow_data_efficiency.csv",
    sep=";",
)
...
for count, loss in enumerate(efficiency_df["loss"].values):
    ...
    base_loss = efficiency_df[
        (efficiency_df["random_seed"] == rs) & (efficiency_df["fraction"] == 100)
    ]["loss"].item()
    loss_ratios.append(loss / base_loss)
...
efficiency_df["n_samples_k"] = [
    str(round(x / 1000, 1)) + "k" for x in efficiency_df["n_samples"].values
]
...
ax2 = ax_list[-1].twiny()
sns.pointplot(
    data=efficiency_df.sort_values(by="n_samples"),
    x="n_samples_k",
    y="loss ratio",
    hue="model",
    palette=palette_2colrs,
...
umap_df["data_set"] = umap_df["data_set"].cat.set_categories(
    ["train", "test (paired)", "test (atac)", "test (rna)"]
)
sns.scatterplot(
    data=umap_df,  # .sort_values(by='data_set'),
    x="UMAP1",
    y="UMAP2",
    hue="data_set",
    palette=extra_palette,
    s=point_size,
    ax=ax_list[-1],
    alpha=0.5,
...
plt.savefig(
    "results/analysis/plots/performance_evaluation/fig_data_efficiency_v2.png",
    dpi=720,
    bbox_inches="tight",
)

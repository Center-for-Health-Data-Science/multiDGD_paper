...
ax_list[-1].set_ylabel("Reconstruction loss")
...
for batch in stages:
    ...
    for model1, model2 in combinations(models, 2):
        ...
        save_batches.append(batch)
        save_model1.append(model1)
        save_model2.append(model2)
        save_n_samples.append(len(group1))
        save_statistic.append(statistic)
        save_p_value.append(p_value)
...
# export a dataframe with the statistics
mwu_df = pd.DataFrame({
    'batch': save_batches,
    'model1': save_model1,
    'model2': save_model2,
    'n_samples': save_n_samples,
    'statistic': save_statistic,
    'p_value': save_p_value
})
mwu_df.to_csv("../results/revision/analysis/batch_integration/mannwhitneyutest_covariateintegration_gastrulation.csv", index=False)
...
mwu_df.to_csv("../results/revision/analysis/batch_integration/mannwhitneyutest_covariateintegration_marrow.csv", index=False)
...
plt.savefig(
    "../results/figures/fig5_final.pdf",
    dpi=300,
    bbox_inches="tight",
    format="pdf",
)
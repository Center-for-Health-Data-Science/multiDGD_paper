...
model._model_name = original_name + "_test10e_noCovError_sZero"
testset = adata[adata.obs["train_val_test"] == "test"].copy()
model.predict_new(testset, include_correction_error=False, start_from_zero=[False,True])
print("   test set inferred")

model._model_name = original_name + "_test50e_noCovError_sZero"
model.predict_new(testset, include_correction_error=False, n_epochs=50, start_from_zero=[False,True])

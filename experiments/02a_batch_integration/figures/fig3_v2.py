...
palette_3colrs = ["lightgray", "#FFD6D6", "darkolivegreen", "firebrick", "midnightblue", "#EC6B6B"]
...
ax_list[-1].legend(
    bbox_to_anchor=(0.62, 0.21), loc="upper left", frameon=False, handletextpad=handletextpad * 4, ncol=2
)  # .set_visible(False)
...
for i in range(len(dkl_annot)):
    if i % 2 == 0:
        ax_list[-1].text(int(i / 2) - 0.35, 1.035, "(" + str(round(dkl_annot[i], 2)) + ", ")
    else:
...
sns.violinplot(data=df_error_ratios,#[df_error_ratios["prediction type"] == "unseen"],
            x="model", y=metric, 
            hue="model", ax=ax_list[-1], 
            linewidth=line_width,linecolor='gray',
            palette=palette_models, dodge=False)
ax_list[-1].legend(
...
metric = "RMSE (rna)"
ax_list[-1].axhline(1.068006157875061,color=palette_models[0],linestyle="--", linewidth=line_width*2)
ax_list[-1].axhline(0.8810019493103027,color=palette_models[1],linestyle="--", linewidth=line_width*2)
sns.boxplot(
...
ax_list[-1].axhline(0.6960756182670593,color=palette_models[0],linestyle="--", linewidth=line_width*2)
ax_list[-1].axhline(0.765972375869751,color=palette_models[1],linestyle="--", linewidth=line_width*2)
sns.boxplot(
    data=df_batch_effect[df_batch_effect["batch"] != "none"],
    x="model",
    y=metric,
...
metric = "1 - ASW"
ax_list[-1].axhline(
    df_batch_effect[(df_batch_effect["model"] != "multiDGD") & (df_batch_effect["batch"] == "none")][metric].mean(),
    color=palette_models[0], linewidth=line_width*2,
    linestyle="--",
)
ax_list[-1].axhline(
    df_batch_effect[(df_batch_effect["model"] == "multiDGD") & (df_batch_effect["batch"] == "none")][metric].mean(),
    color=palette_models[1], linewidth=line_width*2,
    linestyle="--",
)
sns.boxplot(
    data=df_batch_effect[df_batch_effect["batch"] != "none"],
...
ax_list[-1].set_title(
    site + " integration MultiVI + scArches" + "\n(1-ASW = " + str(round(df_batch_effect[(df_batch_effect["model"] != "multiDGD") & (df_batch_effect["batch"] == site)]["1 - ASW"].item(), 4)) + ")"
)
...
plt.savefig("results/analysis/plots/batch_integration/fig3_v2.png", dpi=720, bbox_inches="tight")

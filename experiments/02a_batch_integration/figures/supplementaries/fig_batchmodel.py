...
grid_wspace = 0.5
grid_hspace = 0.5
figure_name = 'results/analysis/plots/fig_batch_supp'
...
#"""
count_type = 'sum'
if not os.path.exists('results/analysis/supplementaries/fig_batch_supp_samples_rna.csv'):
   ...
   if count_type == 'sum':
      ...
      df_temp['n_zeros'] = np.asarray((trainset.X[:, :modality_switch]==0).sum(axis=1)).flatten()
      ...
      for i in range(len(np.unique(sites))):
         ...
         x_rna_site = np.asarray(trainset.X[subset_idx[subset_subset_idx], :modality_switch].todense()).flatten()
   ...
   # save the data
   data_rna.to_csv('results/analysis/supplementaries/fig_batch_supp_samples_rna.csv', index=False)
   print('done')
   exit()
elif not os.path.exists('results/analysis/supplementaries/fig_batch_supp_samples_atac.csv'):
   print('atac')
   trainset, testset, modality_switch, library = load_testdata_as_anndata(data_name)
   print('loaded')
   if count_type == 'sum':
      ...
      for i in range(0, trainset.X.shape[0], 1000):
         ...
         #subset_idx = np.random.choice(trainset.X.shape[0], 1000, replace=False)
         sites = trainset.obs['Site'].values[subset_idx]
         x_atac = np.asarray(trainset.X[subset_idx, modality_switch:].sum(axis=1)).flatten()
         df_temp = pd.DataFrame(x_atac, columns=['counts'])
         x_atac = None
         # get zero counts from sparse matrix
         df_temp['n_zeros'] = np.asarray((trainset.X[subset_idx, modality_switch:]==0).sum(axis=1)).flatten()
         df_temp['batch'] = sites
         if i == 0:
            data_atac = df_temp
         else:
      ...
      for i in range(len(np.unique(sites))):
         site = np.unique(sites)[i]
         subset_subset_idx = np.where(sites==site)[0]
         x_atac_site = np.asarray(trainset.X[subset_idx[subset_subset_idx], modality_switch:].todense()).flatten()
   ...
   trainset, testset, library = None, None, None
   # save the data
   data_atac.to_csv('results/analysis/supplementaries/fig_batch_supp_samples_atac.csv', index=False)
   print('done')
   exit()
else:
   print('already created')
   data_rna = pd.read_csv('results/analysis/supplementaries/fig_batch_supp_samples_rna.csv')
   data_atac = pd.read_csv('results/analysis/supplementaries/fig_batch_supp_samples_atac.csv')
...
sns.histplot(
   ...
   x='counts', hue='batch', bins=100,
   palette=batch_palette, ax=ax_list[-1], alpha=hist_alpha, element='step')
...
if not os.path.exists('results/analysis/supplementaries/fig_batch_supp_samples_pca.csv'):
   step = 10
   trainset, testset, modality_switch, library = load_testdata_as_anndata(data_name)
   print('loaded')
   library = None
   ...
   count_depths_rna = np.asarray(trainset.X[::step,:modality_switch].sum(axis=1))#.flatten()
   #print(count_depths_rna.shape)
   count_depths_atac = np.asarray(trainset.X[::step,modality_switch:].sum(axis=1))#.flatten()
   ...
   pca_data.to_csv('results/analysis/supplementaries/fig_batch_supp_samples_pca.csv', index=False)
else:
   pca_data = pd.read_csv('results/analysis/supplementaries/fig_batch_supp_samples_pca.csv')
pca_data['batch'] = pca_data['batch'].astype("category")
pca_data['batch'] = pca_data['batch'].cat.set_categories(['site1','site2','site3','site4'])
...
pca_data['batch'] = pca_data['batch'].astype("category")
pca_data['batch'] = pca_data['batch'].cat.set_categories(['site1','site2','site3','site4'])
sns.scatterplot(data=pca_data, x='PC1', y='PC2', hue='batch', palette=batch_palette, ax=ax_list[-1], s=point_size, alpha=alpha, linewidth=point_linewidth)
ax_list[-1].legend(bbox_to_anchor=(1.+legend_x_dist, 1.+legend_y_dist),
                  loc='upper left', frameon=False, markerscale=handlesize,
                  handletextpad=handletextpad)
...
sns.scatterplot(data=correction_df, x='D1', y='D2', hue='batch', palette=batch_palette, ax=ax_list[-1], s=point_size, alpha=alpha, linewidth=point_linewidth)
ax_list[-1].legend(bbox_to_anchor=(1.+legend_x_dist, 1.+legend_y_dist),
                  loc='upper left', frameon=False,
                  handletextpad=handletextpad, markerscale=handlesize)
